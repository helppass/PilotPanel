...
app.controller("dcsTest" + "Controller", [
    ...
    function ($scope, $rootScope, $state, $stateParams, $timeout, $interval) {
        ...
        $scope.initPanel = function (name, aircraft) {
            ...
            // 
            $scope.watchData("option_cueing_1", { type: 'string', address: 0x7428, maxLength: 1 }, $scope.onOptionChange, 1);
            $scope.watchData("option_cueing_2", { type: 'string', address: 0x742a, maxLength: 1 }, $scope.onOptionChange, 2);
            $scope.watchData("option_cueing_3", { type: 'string', address: 0x742c, maxLength: 1 }, $scope.onOptionChange, 3);
            $scope.watchData("option_cueing_4", { type: 'string', address: 0x742e, maxLength: 1 }, $scope.onOptionChange, 4);
            $scope.watchData("option_cueing_5", { type: 'string', address: 0x7430, maxLength: 1 }, $scope.onOptionChange, 5);
            // 
            $scope.watchData("option_display_1", { type: 'string', address: 0x7432, maxLength: 4 }, $scope.onOptionChange, 1);
            $scope.watchData("option_display_2", { type: 'string', address: 0x7436, maxLength: 4 }, $scope.onOptionChange, 2);
            $scope.watchData("option_display_3", { type: 'string', address: 0x743a, maxLength: 4 }, $scope.onOptionChange, 3);
            $scope.watchData("option_display_4", { type: 'string', address: 0x743e, maxLength: 4 }, $scope.onOptionChange, 4);
            $scope.watchData("option_display_5", { type: 'string', address: 0x7442, maxLength: 4 }, $scope.onOptionChange, 5);
            // 
        ...
        $scope.onOptionChange = function (a, b, c, d) {
            console.log("onOptionChange", d);
            a = $scope['option_cueing_' + d];
            b = $scope['option_display_' + d];
            if (a == null || b == null)
                return;
            $scope['option_' + d] = a + b;
        }
